...
* [参数说明](#参数说明)
  ...
  * [内置策略参数](#内置策略参数)
    ...
    * [1. BBIKDJSelector（少负战法）](#1-bbikdjselector少负战法)
...
| **`select_stock.py`** | 读取本地 CSV 行情，依据 `strategy_configs.json` 中的 **Selector** 定义批量选股，结果输出到 `select_results.log` 与控制台。                            |
...
python select_stock.py \
  ...
  --config ./strategy_configs.json  # Selector 配置
...
| ------------ | ---------------- | ------------- |
| `--data-dir` | `./data`         | CSV 行情目录      |
| `--config`   | `./strategy_configs.json` | Selector 配置文件 |
| `--date`     | 最新交易日            | 选股日期          |
...
以下参数均来自 **`strategy_configs.json`**，可根据个人喜好自由调整。

#### 1. BBIKDJSelector（少负战法）
...
.
├── appendix.json            # 附加股票池
├── strategy_configs.json             # Selector 配置
