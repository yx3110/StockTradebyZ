#!/usr/bin/env python3
"""
V4.7.2 生产评分器 — V4.6管线 + V4.7.1特征

继承V4.6: ICIR权重裁剪/Meta-Learner推理/Combined Isotonic/增强流动性折扣
新增V4.7.1: 17新特征(财务质量+daily_basic扩展+微观结构+反转+风险)
修复: Bug 3(000300.SH), 去除小盘加成
"""
...
class V472ProductionScorer(V46ProductionScorer):
    """V4.7.2 生产评分器 — V4.6管线 + V4.7.1特征 + 无小盘加成"""

    def __init__(self, model_type: str = 'small_data'):
        self._v472_model_dir = Path(__file__).parent.parent.parent / 'ml_models' / 'trained_models' / 'v472'
    ...
    def _load_models(self):
        """覆盖加载方法, 使用 v472 模型目录"""
        self.model_dir = self._v472_model_dir
        if self.model_type == 'rolling':
            self._load_rolling_models()
        else:
            self._load_v472_model()

    def _load_v472_model(self):
        """加载v4.7.2模型 — V4.6结构 + V4.7.1特征列表"""
        model_files = list(self.model_dir.glob('v472_*.pkl'))
        if not model_files:
            print(f"V4.7.2 未找到模型文件: {self.model_dir}/v472_*.pkl")
            return
        ...
        print(f"V4.7.2 模型加载完成: {list(self.models.keys())} [V4.7.1底座+V4.6管线+{gq_status}]")
    ...
    def _apply_small_cap_bonus(self, results, date, codes):
        """V4.7.2: 不应用小盘加成 (已验证有害)"""
        return results

    # ========== 覆写: predict_scores — V4.6管线 + V4.7.1特征 ==========

    def predict_scores(self, stock_codes: List[str], date: str) -> Dict[str, Dict]:
        """V4.7.2 评分管线:
        Step 1: V4.3 基础特征 (robust z-score + daily_basic + tech)
        Step 1.5: V4.7.1 新特征 (financial + daily_basic_extra + microstructure)
        ...
        if self.feature_cols:
            ...
            if missing:
                if len(missing) > len(self.feature_cols) * 0.3:
                    logger.warning(f"⚠️ V4.7.2: {len(missing)}/{len(self.feature_cols)} 特征缺失: {missing[:5]}...")
                for col in missing:
                    features_df[col] = 0
        ...
        # Step 2: 5/6模型×4目标 base predictions (V4.7.2: 3d=5模型, 其余=6模型含LambdaRank)
