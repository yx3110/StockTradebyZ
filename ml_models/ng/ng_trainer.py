...
class NGTrainer(V485Trainer):
    ...
    def walk_forward_train(self, start_date: str = None, end_date: str = None,
        ...
        if v485_files:
            ...
            model_data['hp_profile'] = getattr(self, '_hp_profile_name', None)
...
if __name__ == '__main__':
    ...
    from ml_models.training.hp_profiles import PROFILES as _HP_PROFILES
    parser.add_argument('--hp-profile', default=None, choices=sorted(_HP_PROFILES),
                        help='超参数试验 profile (见 ml_models/training/hp_profiles.py). '
                             '对 6 成员 ensemble 应用统一的超参数覆盖, 用于 hp sweep. '
                             '默认 None = V4.7.3 现行参数.')
    ...
    def _apply_trainer_switches(trainer, seed_for_subdir=None):
        ...
        if args.hp_profile:
            ...
            logger.info(f"  HP profile: {args.hp_profile} -> {_HP_PROFILES[args.hp_profile]}")
        if args.fast_check:
