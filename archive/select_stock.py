...
def load_config(cfg_path: Path) -> List[Dict[str, Any]]:
    ...
    if not cfgs:
        logger.error("strategy_configs.json 未定义任何 Selector")
        sys.exit(1)
...
def main():
    p = argparse.ArgumentParser(description="Run selectors defined in strategy_configs.json")
    p.add_argument("--data-dir", default="./data", help="CSV 行情目录")
    p.add_argument("--config", default="./strategy_configs.json", help="Selector 配置文件")
