...
"""
批量生成v3.41历史选股报告脚本
用于生成过去8个月的v3.41版本选股数据，然后与v3.0版本进行回测对比
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("v341_batch_generation.log", encoding="utf-8"),
...
def generate_single_day_report(date_str):
    ...
    try:
        ...
        logger.info(f"开始生成 {date_str} 的v3.41选股报告")
        
        # 运行tomorrow_stock_selector.py生成v3.41报告
        cmd = [
            sys.executable, 
            "tomorrow_stock_selector.py", 
            date_str, 
            "--scoring-version", "v3.41"
        ]
        
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=600)
        
        if result.returncode == 0:
            logger.info(f"✅ {date_str} v3.41报告生成成功")
            return {"date": date_str, "status": "success", "message": "报告生成成功"}
        else:
            error_msg = result.stderr[:200] if result.stderr else "Unknown error"
            logger.warning(f"❌ {date_str} v3.41报告生成失败: {error_msg}")
            return {"date": date_str, "status": "failed", "message": error_msg}
            
    except subprocess.TimeoutExpired:
        logger.error(f"⏰ {date_str} v3.41报告生成超时")
        return {"date": date_str, "status": "timeout", "message": "生成超时"}
    except Exception as e:
        logger.error(f"❌ {date_str} v3.41报告生成异常: {e}")
...
def batch_generate_v341_reports(start_date="2025-01-01", end_date="2025-09-02", max_workers=2):
    """批量生成v3.41历史选股报告"""
    
    logger.info("🚀 开始批量生成v3.41历史选股报告")
    ...
    # 确保报告目录存在
    report_dir = "reports/daily_selection_v3.41"
    ...
    results_file = f"v341_batch_generation_results_{datetime.now().strftime('%Y%m%d_%H%M')}.csv"
    ...
    logger.info("🎉 v3.41历史选股报告批量生成完成!")
    return results

def main():
    parser = argparse.ArgumentParser(description='批量生成v3.41历史选股报告')
