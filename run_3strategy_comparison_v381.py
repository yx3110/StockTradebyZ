...
# -*- coding: utf-8 -*-
"""
3策略对比回测 - 保守、平衡、激进策略在V3.81模型上的表现对比
...
strategies = {
    'conservative': {
        'instance': ConservativeStrategy(),
        'name': '保守策略',
        'emoji': '🛡️'
    },
    'balanced': {
        'instance': BalancedStrategy(),
        'name': '平衡策略',
        'emoji': '⚖️'
    },
    'aggressive': {
        'instance': AggressiveStrategy(),
        'name': '激进策略',
        'emoji': '🚀'
    }
}
...
for key, config in strategies.items():
    ...
    print(f"     最大持仓: {strategy.config.max_positions}只, 持仓天数: {strategy.config.max_holding_days}天")
...
for strategy_key, config in strategies.items():
    ...
    try:
        ...
        # 提取V3.7的结果 (修复: 结果在individual_results中)
...
# 创建对比表格
if all_results:
    print(f"\n| {'策略':<12} | {'总收益率':<10} | {'年化收益':<10} | {'夏普比率':<10} | {'最大回撤':<10} | {'胜率':<10} | {'交易次数':<10} |")
    print(f"|{'-'*14}|{'-'*12}|{'-'*12}|{'-'*12}|{'-'*12}|{'-'*12}|{'-'*12}|")

    for result in all_results:
        if 'error' not in result:
            print(f"| {result['emoji']} {result['strategy_name']:<8} | "
...
json_file = report_dir / f"strategy_comparison_{ML_VERSION.replace('.', '')}_{timestamp}.json"
md_file = report_dir / f"strategy_comparison_{ML_VERSION.replace('.', '')}_{timestamp}.md"
