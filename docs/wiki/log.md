...
2026-07-19 | arch   | 生产 regime 风控输入切官方0AMV — `scoring_router._read_amv_regime` 优先读 `market_amv.amv_regime_official` (V11 on 官方序列, `update_official_regime_column` 每日随 compute_and_save 维护), COALESCE 回退 var1 模拟; 旧库无列自动降级。依据: overlay 同构 replay +3.1pp 同 MaxDD + 33 年鲁棒。模型特征 (amv_var1/amv_regime_days) 不动。**⚠️ tushare 000985.SH (中证全指) 2026-06-26 起断供** (其余中证系正常): 外推漂移源切 300/905/852/2000 四指数等权 (一步 MAPE 0.611→0.558% 反而更优, 重拟合 A=0.93526/BETA=1.15/K=5.735e-9); tomorrow_stock_selector 市场环境评分加断供守卫 (指数末日落后 >10 自然日剔除, 000985 曾以 0.3 权重混入陈旧窗口)

2026-07-18 | data   | 官方0AMV(指南针活跃市值)入库+锚定外推 — CSV (1993-01~2026-07, 8165行) 导入 `market_amv_official` 表 (`scripts/import_amv_official.py`, is_simulated=0). 现有模拟 var1 与官方日度动态相关仅 0.74; 新递归模型 `Y_t = 0.93437·Y_{t-1}·(1+1.25·r_中证全指) + 5.802e-9·成交额(元)` (活跃筹码=随大盘漂移+日衰减6.56%+成交注入, `scripts/fit_amv_official.py` 可复现拟合) 日度相关 0.98, 一步MAPE 0.60%, 锚定外推 20天~5%/60天~7% 有界不发散. `indicators/market_amv.extend_amv_official` 挂 compute_and_save 每日自动外推 (is_simulated=1), 重导官方CSV自动覆盖刷新锚点. 7-19 三项回测: 模型切换配置 V5.2 打平但 MaxDD -25→-23%/回撤持续 270→170 天; **生产 v3 overlay 同构 replay 年化 +3.1pp (48.0→51.1%) MaxDD 相同 -9.3% = 纯收益改进**; 33 年大周期择时 (1994-2026 上证) 每次大崩盘都躲过 (2008 -26% vs -69%), 全期 MaxDD -40% vs -72%, V11 规则大周期鲁棒. 详见 `reports/system_evaluation/0AMV官方数据导入与拟合_20260718.md`
