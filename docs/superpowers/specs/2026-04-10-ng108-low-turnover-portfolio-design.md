...
**Date**: 2026-04-10
**Status**: Shipped as ng1.0.8 (2026-04-10, Sharpe=2.52, 换手36x, 详见 log.md)
**Base**: ng1.0.1 信号 (不改) + ng1.0.5 风控 (可选叠加)
**Target**: 换手 44x→15-20x, Sharpe 1.93→2.2+, ~~Pre-2020 保持 A+~~ Pre-2020 保持同向 alpha (2026-04-20 订正, 详见订正说明)
**Principle**: 全部固定参数，无数据驱动优化，零过拟合风险

> ⚠️ **2026-04-20 订正**: 本文档多处"ng1.0.1 Pre-2020 A+"/"双 A+"表述基于当时的 "73.7% A+" 数字, 2026-04-20 审计已证实那是 4-10 评估 bug 遗留 ghost number, 实测 ng1.0.1 bugfix Pre-2020 V5.2=45.5% B。ng1.0.1 是 WF-OOS 单边 A+ 模型, 不是双 A+ 基座。ng1.0.8 的设计原则 (信号不动, 组合层面降换手) 仍然有效, 只是 Pre-2020 成功标准改为"同向 alpha: 净年化 ≥ 0% + 超额胜率 ≥ 60%"。

## 背景

ng1.0.7 迭代教训：从模型信号层面改进(条件标签、市场特征)容易过拟合训练期regime模式(Pre-2020=C级)。ng1.0.1 是当前最强的 WF-OOS 单边 A+ 基座模型 (Pre-2020 实测 B 级, 不是原先声称的双 A+), 信号层面不动。
...
| 年化(净) | 76.9% | **78-82%** | >80% |
| Pre-2020 V5.2 | ~~73.7% A+~~ **45.5% B** (2026-04-20 订正) | **同向 alpha** (净年化 ≥ 0%) | 净年化 ≥ +10% |
