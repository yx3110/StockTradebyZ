...
def _load_json_cached(path_str: str, _mtime_ns: int):
    """按 (path, mtime) 缓存 JSON 报告; 报告写入后不可变, 命中时零 I/O。"""
...
# threaded=True 下 double-checked locking, 避免并发首请求重复实例化 (ML pkl 载入 ~67MB)
...
def get_position_analyzer():
    """获取PositionAnalyzer单例 (DCL)"""
    global _position_analyzer
    if _position_analyzer is None:
        with _singleton_lock:
            if _position_analyzer is None:
                _position_analyzer = PositionAnalyzer(current_app.config['STOCK_DB_PATH'])
    return _position_analyzer


def get_portfolio_manager():
    """获取PortfolioManager单例 (DCL)"""
...
def generate_recommendations():
    ...
    # 一次性批量取最新价并 executemany 更新, 替代逐股 N+1
    codes = [r['code'] for r in analysis_results]
    normalized = [db._normalize_code(c) for c in codes]
    price_map = db._batch_latest_prices(normalized)
    price_updates = []
    for result, ncode in zip(analysis_results, normalized):
        price = price_map.get(ncode)
        pos_id = result.get('position_id')
        if price is not None and pos_id:
            price_updates.append((price, pos_id))
    if price_updates:
        with db.get_webapp_db_connection() as conn:
            conn.executemany(
    ...
    # 获取ML版本信息
    from config import ML_VERSION_DESCRIPTIONS
    ml_version = analyzer.ml_version or 'N/A'
    ml_info = ML_VERSION_DESCRIPTIONS.get(ml_version, '')
...
def run_evaluations():
    ...
    # 一次性批量预取: 已评估的 (trade_id, days_after) + 全量股票最新价 (用于 no_price_data 区分)
    evaluated_map = db.get_evaluated_days_by_trade([t['id'] for t in trades])
    latest_price_map = db._batch_latest_prices(
        [db._normalize_code(t['code']) for t in trades])

    for trade in trades:
        # 检查是否已评估 (改为 dict 查找, 避免 N+1)
        if days_after in evaluated_map.get(trade['id'], ()):
        ...
        if not price_history:
            # 区分：完全无价格数据 vs 交易太新 (用预取的 latest_price_map, 避免 N+1)
            if latest_price_map.get(db._normalize_code(trade['code'])) is None:
...
def get_groups():
    ...
    # 一次拉全部持仓，按 group_id 聚合，避免 N+1
    all_positions = db.get_all_positions()
    agg = {}
    for p in all_positions:
        gid = p.get('group_id')
        slot = agg.setdefault(gid, {'count': 0, 'mv': 0.0})
        slot['count'] += 1
        slot['mv'] += p.get('market_value') or 0
    for group in groups:
        slot = agg.get(group['id'], {'count': 0, 'mv': 0.0})
        group['position_count'] = slot['count']
        group['total_market_value'] = slot['mv']
...
def get_today_selections():
    ...
    for rd in report_dirs:
        if not rd.exists():
            continue
        for f in rd.glob('analysis_data_*.json'):
    ...
    try:
        mtime = latest_file.stat().st_mtime_ns
    except OSError:
        mtime = 0
    report_data = _load_json_cached(str(latest_file), mtime)
...
def get_rebalance_suggestions():
    ...
    codes = [s['code'] for s in suggestions if s.get('code')]
    name_by_code = db.get_stock_names_batch(codes)
    for s in suggestions:
        code = s.get('code') or ''
        s['stock_name'] = name_by_code.get(db._normalize_code(code), '')
